...
def get_text():
    ...
    time.sleep(5)  # chương trình sẽ tạm dừng trong 5s
    stop()
    return 0
...
def help_me():
    speak("""Lee có thể giúp bạn thực hiện các việc sau đây:
    <<........>>>
    1. chào hỏi
    2. Hiển thị giờ
...
def hello():
    ...
    if 0 <= day_time < 11:
        speak(f'chào bạn. chúc bạn một buổi sáng tốt lành <..>')
    elif 11 <= day_time < 13:
        speak(f'chào bạn. buổi trưa tốt lành nha <..>')
    elif 13 <= day_time < 18:
        speak(f'chào bạn. buổi chiều an lành <..>')
    elif 18 <= day_time < 22:
        speak(f'chào bạn. buổi tối vui vẻ, bạn đã ăn cơm chưa <..>')
    elif 22 <= day_time < 23:
        speak(f'chào bạn. muộn rồi bạn nên đi ngủ, ngủ sớm để đẹp da nhé <..>')
    else:
        speak(f'chào bạn. buổi tối vui vẻ <..>')
...
class Widget: #GUI của project và các lệnh kèm theo
    def __init__(self):
        root = Tk()
        root.title('Lee')
        root.geometry('700x470')

        img = ImageTk.PhotoImage(Image.open('Lee.jpg'))
        ...
        userFrame = LabelFrame(root, text='Lee', font=('Railways', 24, 'bold'))
        userFrame.pack(fill='both', expand='yes')

        top = Message(userFrame, textvariable=userText, bg='#021C1E', fg='white')
...
bg='#2C7873', fg='white', command=self.clicked).pack(fill='x', expand='no')
        btn2 = Button(root, text='Close', font=('railways', 10,
'bold'), bg='#6FB98F', fg='black', command=root.destroy).pack(fill='x', expand='no')
        speak('Chào bạn, bạn cần mình giúp gì ạ')
        root.mainloop()

    def clicked(self):
        text = get_text()
        text = text.lower()
        if "tạm biệt" in text or "thôi" in text:
                stop()
        if 'chào' in text:
            hello()
        if 'làm gì' in text:
            help_me()
        if 'bây giờ' or 'hôm nay' in text:
            get_time(text)
        if "mở" in text:
            if "ứng dụng" in text:
                open_app(text)
        ...
        if "đọc báo" in text:
            ...
            if input("Để tiếp tục y/n: ") == "y":
                pass
        if "định nghĩa" in text:
            tell_me_about()
#        else:
#            speak(f'chức năng này chưa có bạn vui lòng chọn lại sau nha <..>')
